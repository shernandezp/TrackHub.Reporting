...
public class LiveReport(IRouterReader reader, IExcelHelper helper) : IReport
{
    public string ReportCode => Reports.LiveReport;
    public async Task<byte[]> GenerateAsync(FilterDto filters, CancellationToken cancellationToken)
    {
        var data = await reader.GetDevicePositionsAsync(cancellationToken);
